...
class TimeSeriesTransformerClassifier(pl.LightningModule):
    ...
    def forward(self, x, return_attn=False):
        ...
        # x_channel = self.channel_transformer(x_channel)  # [B, C, T]
        channel_attn_weights = []
        for layer in self.channel_layers:
            x_channel, attn = layer(x_channel)
            channel_attn_weights.append(attn)  # attn shape: [B, nhead, C, C]
        x_channel_pooled = x_channel.mean(dim=2)         # [B, C]

        # # === Gated Fusion ===
        # gate = self.gate(x_time_pooled)            # [B, C]
        # x_fused = gate * x_time_pooled + (1 - gate) * x_channel_pooled  # [B, C]
        # x_fused = self.dropout(x_fused)

        # return self.fc(x_fused)                    # [B, num_classes]
        # === Gated Fusion (Softmax version) ===
